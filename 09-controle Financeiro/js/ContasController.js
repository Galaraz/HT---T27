class ContasController {
  ...
  abrirMenu() {
    ...
    document
      ...
      .classList.remove("btn-menu-abrir-exibir");
  ...
  atualizarEstado() {
    ...
    if (localStorage.getItem("contas") != null) {
      ...
      for (let i = 0; i < this.contas.length; i++) {
        if (this.contas[i].ativa) {
          let op = document.createElement("option");
          op.setAttribute("value", this.conta[i].id);
          select.appendChild(op);
        }
      }
    }
  ...
  gerarTabela() {
    ...
    for (let i = 0; i < this.contas.length; i++) {
      ...
      imgEditar.setAttribute(
        "onclick",
        `contasController.editar('${this.contas[i].id}')`
      );
      ...
      imgExcluir.setAttribute(
        "onclick",
        `contasController.excluir('${this.contas[i].id}')`
      );
  ...
  editar(id) {
    ...
    while (i < this.contas.length && !achou) {
      if (this.contas[i].id == id) {
        document.getElementById("valor").value = this.contas[i].valor;
        document.getElementById("tipo").value = this.contas[i].tipo;
        document.getElementById("paga").checked = this.contas[i].paga;
        document.getElementById("receptor").value = this.receptor[i].receptor;
